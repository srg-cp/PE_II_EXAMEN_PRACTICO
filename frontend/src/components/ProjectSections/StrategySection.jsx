...
const StrategySection = ({ projectId, sectionData, onDataUpdate, user, project }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await axios.put(`/api/projects/${projectId}/sections/strategy`, {
        content,
        userId: user._id
      });
      ...
      setError(error.response?.data?.message || 'Error al guardar la estrategia');
    } finally {
      setSaving(false);
    }
  };
      ...
      <Box display="flex" gap={2} mb={3}>
        {!isEditing ? (
          <Button
            ...
            disabled={!canEdit}
          >
            Editar Identificación de Estrategia
          </Button>
        ) : (
          <>
            <Button
